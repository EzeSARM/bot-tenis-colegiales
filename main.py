...
def obtener_estado_turnos():
    """Realiza la búsqueda y retorna los turnos divididos entre semana y fin de semana."""
    ...
    while True:
        ...
        if tiempo_actual - ULTIMO_ESCANEO >= INTERVALO_ESCANEO:
            ...
            if lineas_nuevas_finde:
                ...
                TURNOS_NOTIFICADOS.update(turnos_visibles)
                print(f"✅ Notificación enviada: {len(lineas_nuevas_finde)} turno(s) de fin de semana.")
            ...
            # Prioridad 2: Notificación estándar si solo hay turnos de DÍAS DE SEMANA
                ...
                enviar_mensaje_telegram(mensaje_alerta)
                TURNOS_NOTIFICADOS.update(turnos_visibles)
                print(f"✅ Notificación enviada: {len(lineas_nuevas_semana)} grupo(s) de turnos nuevos.")
            else:
